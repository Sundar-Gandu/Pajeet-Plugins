...
public interface OneClickThievingConfig extends Config {
    @ConfigItem(keyName = "clickOverride", name = "Clicks Override", description = "Makes all of your clicks pickpocket chosen NPC", position = -2)
    ...
    @ConfigItem(keyName = "blackjackFix", name = "Blackjack Fix", description = "This will try to prevent you from dying while left click blackjacking", position = 10)
    default boolean enableBlackjackFix() {
        return false;
    }
    @ConfigItem(keyName = "blackjackResetFood", name = "Blackjack reset food", description = "This is used in blackjacking only. Toggle on and back off when you have restocked food.", position = 11)
